/* ST Microelectronics FXLS8974CF 3-axis accelerometer driver
 *
...
#define DT_DRV_COMPAT st_fxls8974cf
...
static inline void fxls8974cf_convert(struct sensor_value *val, int32_t raw_val
				 )
{

	val->val1 = ((raw_val * SENSOR_G) / 32) / 1000000;;
	val->val2 = ((raw_val * SENSOR_G) / 32) % 1000000;;
}



static int fxls8974cf_channel_get(const struct device *dev,
			      ...
			      struct sensor_value *val)
{
	...
	fxls8974cf_convert(val, data->bufx[data->selected_range]);
	data->selected_range++;
	break;
	case SENSOR_CHAN_ACCEL_Y:
	fxls8974cf_convert(val,data->bufy[data->selected_range]);
	data->selected_range++;
	break;
	case SENSOR_CHAN_ACCEL_Z:
	fxls8974cf_convert(val,data->bufz[data->selected_range]);
	data->selected_range++;
	break;
...
uint8_t set_mode(struct fxls8974cf_device_config *fxls8974cf , fxls8974cf_mode_t mode, uint8_t* ctrl1)
{
...
static int fxls8974cf_read_accel_raw(struct fxls8974cf_device_config *fxls8974cf , uint8_t nRead, fxls8974cf_accel_raw_t* pAccelData)
{
    if(NULL == fxls8974cf)
	{
	...
	status  = sensor_comm_read(&fxls8974cf->i2c, FXLS8974CF_OUT_X_LSB, nRead, (uint8_t*)pAccelData);
   // Return the status.
    return status;
}


static int  fxls8974cf_configure_accel( struct fxls8974cf_device_config *pDriver, fxls8974cf_odr_t odr, fxls8974cf_accel_config_t *pConfig )
{
	...
	//Read the CRTL1 and preserve the existing configuration bits of the control registers other than standby bit
    ...
    if (NULL != pConfig)
	{
		...
		// Configure the fifo
		status  = sensor_comm_write(&pDriver->i2c, FXLS8974CF_BUF_CONFIG1, 1, &pConfig->fifosetup.config1.w);
		status  = sensor_comm_write(&pDriver->i2c, FXLS8974CF_BUF_CONFIG2, 1, &pConfig->fifosetup.config2.w);
...
static int fxls8974cf_attr_set(const struct device *dev, enum sensor_channel chan,
...
static int fxls8974cf_sample_fetch(const struct device *dev,
			       enum sensor_channel chan)
{
	...
	/* fetch raw data sample */
       ...
       data->bufx[samples_count]= ((int16_t)accel_data.accel[1] << 8) | (int16_t)accel_data.accel[0];
       data->bufy[samples_count]= ((int16_t)accel_data.accel[3] << 8) | (int16_t)accel_data.accel[2];
       data->bufz[samples_count]= ((int16_t)accel_data.accel[5] << 8) | (int16_t)accel_data.accel[4];

	return 0;
}

static const struct sensor_driver_api fxls8974cf_driver_api = {
	.attr_set = fxls8974cf_attr_set,
#if CONFIG_FXLS8974CF_TRIGGER
	.trigger_set = fxls8974cf_trigger_set,
#endif /* CONFIG_FXLS8974CF_TRIGGER */
...
#define FXLS8974CF_DEFINE(inst)																\
	...
	static const struct fxls8974cf_device_config fxls8974cf_device_config_##inst = {		\
		...
		IF_ENABLED(CONFIG_FXLS8974CF_TRIGGER,												\
			   (.int_gpio = GPIO_DT_SPEC_INST_GET_OR(inst, drdy_gpios, { 0 }),))			\
	};																						\
	\
	SENSOR_DEVICE_DT_INST_DEFINE(inst, fxls8974cf_init, NULL,								\
			&fxls8974cf_data_##inst, &fxls8974cf_device_config_##inst, POST_KERNEL,	\
